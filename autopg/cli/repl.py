"""
REPL interface — AutoPG output style: ⏺ tool calls, ⎿ results, timing.
"""
...
class ReplInterface:
    """REPL with AutoPG output format."""
    ...
    def _get_history_path(self) -> str:
        d = os.path.expanduser("~/.autopg"); os.makedirs(d, exist_ok=True)
    ...
    async def run(self):
        ...
        @kb.add("enter")
        def submit(event):
            """Enter = submit message (AutoPG behavior)."""
        ...
        while not self.should_exit:
            try:
                ...
                if self.cmd and user_input.strip().startswith("/"):
                    ...
                    # Interactive session picker (AutoPG /resume)
                    if ctx.get("open_session_picker"):
                        await self._do_resume()
                        continue

                    # Prompt injection (AutoPG: PromptCommand) — e.g. /review /diff /commit
    ...
    def _render_history(self, messages: list):
        """Render restored session history in AutoPG format.
    ...
    async def _do_resume(self):
        ...
        # Render restored history in AutoPG format
    ...
    def _on_permission_check(self, tool_name: str, is_destructive: bool, activity: str) -> bool:
        """Interactive permission confirmation (AutoPG: PermissionDialog)."""
        ...
        try:
            async for event in self.engine.submit_message(prompt):
                ...
                if etype == "token":
                    ...
                    # Render the streamed text as formatted Markdown (AutoPG style)
                    sys.stdout.write("\n")
                    sys.stdout.flush()
    ...
    def _format_result(self, name: str, data: str) -> str:
        """Format result as single line summary, matching AutoPG style."""
    ...
    def _print_welcome(self):
        ...
        self.console.print(Panel(
            f"[bold]AutoPG v{__version__}[/bold]\n"
            f"{provider} · {self.config.model} · {self.config.permission_mode} mode\n"
            "Type [bold]/help[/bold] for commands.",
            border_style="bold blue", title="AutoPG",
        ))

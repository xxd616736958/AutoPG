...
@dataclass
class SkillDef:
    """One skill definition. Same structure as AutoPG's PromptCommand skill."""
    name: str
    description: str
...
def load_all_skills(project_root: str = None) -> SkillRegistry:
    ...
    # 2. User skills (~/.autopg/skills/)
    user_dir = os.path.join(os.path.expanduser("~/.autopg"), "skills")
    for skill in load_skills_from_dir(user_dir, "user"):
        skill_registry.register(skill)

    # 3. Project skills (.autopg/skills/) (highest priority)
    cwd = project_root or os.getcwd()
    project_dir = os.path.join(cwd, ".autopg", "skills")

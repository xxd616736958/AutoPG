"""
User hooks — AutoPG hooks system. Shell commands triggered by agent events.
Configuration: ~/.autopg/config.json → "hooks" field.
"""
import os, json, asyncio, fnmatch, logging, time
from typing import Optional

logger = logging.getLogger(__name__)
hook_logger = logging.getLogger("autopg.hooks")  # Separate file for audit


def load_hooks_config() -> dict:
    """Load hooks from ~/.autopg/config.json."""
...
def build_hook_env(tool_name: str, tool_args: dict) -> dict:
    """Build environment variables for hook execution (AutoPG match)."""
    env = {
        "AUTOPG_TOOL_NAME": tool_name,
        "AUTOPG_TOOL_INPUT": json.dumps(tool_args),
    }
    if "file_path" in tool_args:
        env["AUTOPG_FILE_PATH"] = tool_args["file_path"]
    if "command" in tool_args:
        env["AUTOPG_COMMAND"] = tool_args["command"]
    return env

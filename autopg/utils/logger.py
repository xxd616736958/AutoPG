"""
Centralized logging setup for AutoPG.
...
def setup_logging(
    ...
    """Configure AutoPG logging. Call once at startup.

    Args:
        level: Override log level (DEBUG/INFO/WARNING/ERROR). Default: INFO.
        log_dir: Override log directory. Default: ~/.autopg/logs/
    """
    level = level or os.environ.get("AUTOPG_LOG_LEVEL", "INFO")
    log_dir = log_dir or os.environ.get(
        "AUTOPG_LOG_DIR",
        os.path.join(os.path.expanduser("~/.autopg"), "logs"),
    )
    os.makedirs(log_dir, exist_ok=True)

    root = logging.getLogger("autopg")
    root.setLevel(getattr(logging, level.upper(), logging.INFO))

    # ── File handler: system log (10MB × 5, INFO+) ──
    sys_log = os.path.join(log_dir, "AutoPG.log")
    ...
    hook_logger = logging.getLogger("autopg.hooks")

...
class McpServerConfig:
    """One MCP server config. Matching AutoPG's McpServerConfig."""
    name: str
...
def load_builtin_postgres_mcp() -> dict[str, McpServerConfig]:
    ...
    """
    enabled = os.environ.get("AUTOPG_ENABLE_POSTGRES_MCP", "true").lower()
    if enabled in ("0", "false", "no", "off"):
        return {}

    database_uri = os.environ.get("AUTOPG_DATABASE_URI") or os.environ.get("DATABASE_URI")
    if not database_uri:
        user = os.environ.get("USER") or "postgres"
        database_uri = f"postgresql://{user}@localhost:5432/db_agent"

    access_mode = os.environ.get("AUTOPG_POSTGRES_ACCESS_MODE", "restricted")
    command = os.environ.get("AUTOPG_POSTGRES_MCP_COMMAND", "python")
    package = os.environ.get("AUTOPG_POSTGRES_MCP_PACKAGE", "-m")
    entrypoint = os.environ.get("AUTOPG_POSTGRES_MCP_ENTRYPOINT", "postgres_mcp")
...
def load_from_config_json() -> dict[str, McpServerConfig]:
    """Read mcpServers from ~/.autopg/config.json."""
    config_path = os.path.join(os.path.expanduser("~/.autopg"), "config.json")
    try:
...
def load_from_project_mcp(project_root: str = None) -> dict[str, McpServerConfig]:
    """Read .autopg/.mcp.json from project root."""
    cwd = project_root or os.getcwd()
    mcp_path = os.path.join(cwd, ".autopg", ".mcp.json")
    try:

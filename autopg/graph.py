    ...
    *,
    ...
    async def _tool_hook_wrapper(request, execute):
        ...
        if tool_name == "skill":
            ...
            if skill_name:
                marker = f"AUTOPG_SKILL_RUNNING_{skill_name}"
                try:
                    import os, time
    ...
    def _skill_injection_messages(messages: list) -> list[HumanMessage]:
        ...
        for msg in messages:
            ...
            if isinstance(content, str) and "<!-- AutoPG-skill-loaded:" in content:
                start = content.find("<!-- AutoPG-skill-loaded:") + len("<!-- AutoPG-skill-loaded:")
        ...
        for msg in messages:
            ...
            injected.append(HumanMessage(content=(
                f"<!-- AutoPG-skill-loaded:{skill_name} -->\n"
...
def create_checkpointer(backend: str = "sqlite", **kwargs):
    ...
    if backend == "sqlite":
        ...
        db_path = kwargs.get("db_path") or os.path.join(
            os.path.expanduser("~/.autopg"), "checkpoints", "sessions.db"
        )
...
def load_checkpointer_from_config(config: dict = None):
    ...
    if config is None:
        try:
            with open(os.path.join(os.path.expanduser("~/.autopg"), "config.json")) as f:
                config = json.load(f)
        except Exception:
            config = {}

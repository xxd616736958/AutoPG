...
async def task_create(subject: str, description: str) -> str:
    """Create a new task for tracking progress on complex tasks."""
...
async def task_update(task_id: str, status: str = None) -> str:
...
class TaskListInput(BaseModel): pass
...
@tool(args_schema=TaskStopInput)
async def task_stop(task_id: str) -> str:
    """Stop a running background task."""
    t = _store.get(task_id)
    if t: t["status"] = "stopped"; return json.dumps({"id": tid, "status": "stopped"})
...
@tool(args_schema=TaskOutputInput)

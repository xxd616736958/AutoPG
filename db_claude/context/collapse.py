"""
Context Collapse — Claude Code's selective span-folding system.
Replaces specific conversation spans with summaries, keeping other messages intact.

Key design (matching Claude Code's contextCollapse):
...
class CollapseCommit:
    """One collapse operation. Claude Code: commit log entry in collapse store."""
    commit_id: str
...
class ContextCollapseManager:
    """
    Manages selective span collapses. Claude Code: contextCollapse module.
    ...
    def __init__(self, session_id: str, provider: str = "deepseek",
        ...
        # Stats (Claude Code: collapse stats)
        self.stats = {
            "collapsed_spans": 0,
            "staged_spans": 0,
            "health": {
    ...
    def _store_path(self) -> str:
        base = os.environ.get("DB_CLAUDE_CONFIG_DIR", os.path.expanduser("~/.db-claude"))
    ...
    # ── Staging (Claude Code: collapse classifier identifies collapsible spans) ──
    ...
    def stage_collapse(self, messages: list[BaseMessage],
        ...
        Claude Code: collapse classifier marks spans as collapsible.

        Heuristic rules (Claude Code's classifier approximation):
    ...
    def auto_stage_candidates(self, messages: list[BaseMessage], turn: int):
        ...
        Claude Code: collapse classifier in query loop.
        """
    ...
    async def commit_staged(
        ...
        Claude Code: commit collapses in batch (amortize cache miss).
    ...
    async def _generate_span_summary(self, span: list[BaseMessage]) -> str:
        ...
        Claude Code: internal spawn for collapse summary generation.
        """
    ...
    # ── Project view (Claude Code: projectView — idempotent read-time projection) ──

    def project_view(self, messages: list[BaseMessage]) -> list[BaseMessage]:
        """
        Replay commit log over current messages. Idempotent.
        Claude Code: replayed on every entry point so one-off.
    ...
    # ── Apply (Claude Code: applyCollapsesIfNeeded — runs before API call) ──
    ...
    async def apply_collapses_if_needed(
        ...
        the context limit. Claude Code: runs BEFORE auto-compact.
    ...
    # ── Recovery (Claude Code: recoverFromOverflow — 413 drain) ──
    ...
    def recover_from_overflow(self, messages: list[BaseMessage]) -> dict:
        ...
        Claude Code: first recovery path before reactiveCompact.
    ...
    # ── Withhold (Claude Code: isWithheldPromptTooLong — 413 intercept in streaming) ──
    ...
    def is_withheld_prompt_too_long(self, message, is_prompt_too_long_fn) -> bool:
        ...
        Claude Code: allows collapse drain to run before surfacing 413.
        """
    ...
    def is_enabled(self) -> bool:
        """Claude Code: isContextCollapseEnabled — feature gate."""
        return True  # Always enabled in db-claude

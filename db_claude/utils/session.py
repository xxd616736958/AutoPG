"""
Session persistence — Claude Code's recordTranscript architecture.
...
def get_transcript_path(session_id: str) -> str:
    """Claude Code: getTranscriptPathForSession()"""
    return os.path.join(get_sessions_dir(), f"{session_id}.jsonl")

# ── Serialization (matching Claude Code's SerializedMessage) ──
...
# ── Write (Claude Code: enqueueWrite / recordTranscript) ──
...
async def _flush_deferred(filepath: str):
    """Flush buffered entries to disk after 100ms (Claude Code's lazy flush)."""
...
def enqueue_write(session_id: str, entry: dict, await_flush: bool = False):
    ...
    Fire-and-forget by default (matches Claude Code's lazy flush for UI responsiveness).
...
async def record_transcript(session_id: str, messages: list, await_write: bool = False):
    """Write multiple messages to transcript. Claude Code: recordTranscript().
...
# ── Metadata (Claude Code: reAppendSessionMetadata) ──
...
# ── Read / Resume (Claude Code: hydrateFromTranscript) ──
...
def list_sessions(limit: int = 50) -> list[dict]:
    """List sessions with metadata, newest first. Claude Code: listSessions()."""
    _ensure_dir()
    files = sorted(
...
def resume_messages(session_id: str) -> list:
    """Restore LangChain messages from transcript. Claude Code: hydrateSession()."""
...
def save_session(session_id: str, messages: list, metadata: dict = None, *, await_write: bool = False):
    ...
    Claude Code: combination of recordTranscript + reAppendSessionMetadata."""
    _ensure_dir()
